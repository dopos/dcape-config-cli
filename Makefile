...
ls:
	...
	curl -gs -H "X-narra-token: $$CIS_TOKEN" "$${ENFIST_URL}/tag" | jq -r '.result[].code'
...
cat:
	...
	@echo "Getting env into $(TAG)" \
	  && tag=$${TAG%.env} \
	  && curl -gs -H "X-narra-token: $$CIS_TOKEN" "$${ENFIST_URL}/tag_vars?a_code=$$tag" \
...
set:
	...
	@echo "Setting $(TAG) from file" \
	  ...
	  && jq -R -sc ". | {\"a_code\":\"$$tag\",\"a_data\":.}" < $${tag}.env \
	    | curl -gsd @- -H "X-narra-token: $$CIS_TOKEN" "$${ENFIST_URL}/tag_set" | jq '.[0]'
...
del:
	...
	@echo "Deleting $(TAG)" \
	  && tag=$${TAG%.env} \
	  && curl -gs -H "X-narra-token: $$CIS_TOKEN" "$${ENFIST_URL}/tag_del?a_code=$$tag" | jq '. .result[0]'
